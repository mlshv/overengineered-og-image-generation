...
export async function generate({
  ...
  return new ImageResponse(
      ...
      <div tw="flex items-center justify-center bg-white w-full h-full p-8">
        ...
        >
          <div tw="flex items-center justify-center">
            <div tw="flex rounded-full overflow-hidden w-100 h-100">
              <img src={user.avatar} alt={user.name} tw="h-full" />
            </div>
          </div>
          <div tw="flex flex-col items-center" style={{ gap: 32 }}>
            <div tw="flex flex-col items-center text-center font-bold text-6xl">
              <div>{user.name}</div>
              <div>invites you to join</div>
            </div>
            <div tw="text-3xl text-gray-500 text-center">
    ...
    {
      ...getImageSizeByAspectRatio("1x1.91"),
      fonts,
    }
  );
}
